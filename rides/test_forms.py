...
class TestRideRequestForm(TestCase):
    ...
    def test_seats_requested_field_validation(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when seats_requested is out of range"
        )
        self.assertIn(
            'seats_requested',
            form.errors,
            msg="seats_requested field should have errors when invalid"
        )
    ...
    def test_form_invalid_without_required_fields(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when required fields are missing"
        )
        self.assertIn(
            'seats_requested',
            form.errors,
            msg="seats_requested field should have errors when missing"
        )
...
class TestRideSearchForm(TestCase):
    ...
    def test_min_passengers_validation(self):
        ...
        form = RideSearchForm(
            data={**self.valid_data, 'min_passengers': 'invalid'}
        )
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when min_passengers is not an integer"
        )
        self.assertIn(
            'min_passengers',
            form.errors,
            msg="min_passengers field should have errors when invalid"
        )
    ...
    def test_search_form_invalid_data(self):
        """
        Test that form is valid with empty origin and destination.
        """
        form = RideSearchForm(data=self.invalid_data)
        self.assertTrue(
            form.is_valid(),
            msg="Form should be valid when origin and destination are empty"
        )

    def test_search_form_partial_data(self):
        """
        Test that form is valid with partial data.
        """
        form = RideSearchForm(data=self.partial_data)
        self.assertTrue(
            form.is_valid(),
            msg="Form should be valid when destination is empty"
        )
    ...
    def test_min_passengers_boundary_values(self):
        ...
        self.assertTrue(
            form.is_valid(),
            msg="Form should be valid when min_passengers is 1"
        )
        form = RideSearchForm(data={**self.valid_data, 'min_passengers': 5})
        self.assertTrue(
            form.is_valid(),
            msg="Form should be valid when min_passengers is 5"
        )
        form = RideSearchForm(data={**self.valid_data, 'min_passengers': 0})
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when min_passengers is 0"
        )

    def test_origin_destination_alphabetic_validation(self):
        """
        Test that origin and destination must be alphabetic.
        """
        form = RideSearchForm(data={**self.valid_data, 'origin': 'Leiden123'})
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when origin contains numbers"
        )
        self.assertIn(
            'origin',
            form.errors,
            msg="origin field should have errors when containing numbers"
        )
        form = RideSearchForm(
            data={**self.valid_data, 'destination': 'Delft!@#'}
        )
        self.assertFalse(
            form.is_valid(),
            msg=(
                "Form should be invalid when destination "
        ...
        self.assertIn(
            ...
            form.errors,
            msg=(
                "destination field should have errors "
    ...
    def test_search_form_date_in_past(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when date is in the past"
        )
        self.assertIn(
            'date',
            form.errors,
            msg="date field should have errors when in the past"
        )
...
class TestRideForm(TestCase):
    ...
    def test_origin_min_length_validation(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when origin is less than 3 characters"
        )
        self.assertIn(
            'origin',
            form.errors,
            msg="origin field should have errors when too short"
        )
    ...
    def test_destination_min_length_validation(self):
        ...
        self.assertIn(
            'destination',
            form.errors,
            msg="destination field should have errors when too short"
        )
    ...
    def test_origin_destination_alphabetic_validation(self):
        ...
        form = RideForm(
            data={**self.valid_data, 'destination': 'Rotterdam!@#'}
        )
        self.assertFalse(
            form.is_valid(),
            msg=(
    ...
    def test_date_required_validation(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when date is missing"
        )
        self.assertIn(
            'date',
            form.errors,
            msg="date field should have errors when missing"
        )
    ...
    def test_seats_available_min_max_validation(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when seats_available is less than 1"
        )
        self.assertIn(
            'seats_available',
            form.errors,
            msg="seats_available field should have errors when less than 1"
        )
        form = RideForm(data={**self.invalid_data, 'seats_available': 5})
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when seats_available is greater than 4"
        )
        self.assertIn(
            'seats_available',
            form.errors,
            msg="seats_available field should have errors when greater than 4"
        )
    ...
    def test_form_invalid_data(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid with incorrect data"
        )
    ...
    def test_origin_whitespace_only(self):
        ...
        self.assertFalse(
            form.is_valid(),
            msg="Form should be invalid when origin is only whitespace"
        )
        self.assertIn(
            'origin',
            form.errors,
            msg="origin field should have errors when only whitespace"
        )
    ...
    def test_max_seats_parameter_limits_choices(self):
        ...
        choices = [
            choice[0] for choice in form.fields['seats_requested'].choices
        ]
        self.assertEqual(
            choices,
            ['1', '2'],
            msg="Choices should be limited to ['1', '2'] when max_seats=2"
        )
