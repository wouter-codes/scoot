...
# These tuples define the valid options for seats,
# ride status, and request status.
...
class RidesQuerySet(models.QuerySet):
    ...
    def apply_search_filters(self, form):
        ...
        if form.cleaned_data.get('date'):
            queryset = queryset.filter(
                date__date=form.cleaned_data['date']
            )
        if form.cleaned_data.get('min_passengers'):
            queryset = queryset.filter(
                seats_available__gte=form.cleaned_data['min_passengers']
            )
...
class Rides(models.Model):
    ...
    seats_available = models.IntegerField(
        choices=SEATS_AVAILABLE, default=1
    )
    pickup_notes = models.TextField()
    status = models.CharField(
        max_length=1, choices=RIDES_STATUS, default='0'
    )
    ...
    def clean(self):
        ...
        super().clean()
        if (
            self.origin and self.destination and
            self.origin.lower() == self.destination.lower()
        ):
            raise ValidationError(
                'Origin (pick up) and destination '
                '(drop off) must be different.'
            )

    class Meta:
        ordering = ['-created_on']
        verbose_name_plural = (
            'Rides'  # ensures the plural form is correct in admin interface
        )

    def __str__(self):
        date_str = self.date.strftime('%Y-%m-%d %H:%M') if self.date else 'TBD'
        return (
            f"{self.origin} to {self.destination} on {date_str} "
            f"- {self.seats_available} seats"
        )


class RideRequest(models.Model):
    """
    Stores a ride request entry related to :model:'auth.User' and
    :model:'Rides'.
    """
    passenger = models.ForeignKey(
        User, on_delete=models.CASCADE,
        related_name='ride_requests_as_passenger'
    )
    ride = models.ForeignKey(
        Rides, on_delete=models.CASCADE,
    ...
    status = models.CharField(
        max_length=1, choices=REQUEST_STATUS, default='0'
    )
    created_on = models.DateTimeField(auto_now_add=True)
    updated_on = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return (
            f"Ride ID:{self.ride.id} | RideRequest by "
            f"{self.passenger.username} for ride from "
            f"{self.ride.origin} to {self.ride.destination} "
