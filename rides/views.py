...
from .forms import (
    RideSearchForm, RideForm,
    RideRequestForm, RideRequestEditForm
)
...
def homepage(request):
    ...
    search_rides.html for authenticated users.
    """
...
def search_rides(request):
    """
    Display the home page with the ride search form
    and filtered ride results.
    Applies search filters, excludes rides created or
    already requested by the user, and orders results by date.
    ...
    # Exclude rides created by the logged-in user and
    # rides already requested by the user
    if request.user.is_authenticated:
        ...
        rides = rides.exclude(id__in=user_request_ids)
    else:
        user_request_ids = set()
...
def my_rides(request):
    ...
    all_rides = all_rides.annotate(
        request_count=Count(
            'ride_requests',
            filter=Q(ride_requests__status='0')
        )
    )
...
def ride_detail(request, ride_id):
    """
    Display full details for a single ride, including
    ride info and ride requests. If the user is the driver,
    show all requests for this ride. If the user is a
    passenger, show their own request if it exists.
    Indicates if any requests have been accepted.
    """
...
def request_ride(request, ride_id):
    ...
    Users select number of seats and confirm.
    ...
    if not request.user.is_authenticated:
        ...
        message = (
            f'You need to be logged in to request a ride. Sign up or '
            f'<a href="{login_url}" class="link">log in</a> to continue.'
        )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ride_request.save()
            return redirect(
                'ride_request_confirmation', request_id=ride_request.id
            )
        else:
            context = {'ride': ride, 'form': form}
            return render(request, 'rides/request_ride.html', context)
...
def ride_request_confirmation(request, request_id):
    """
    Display booking confirmation page after a
    successful ride request submission. Only accessible
    to the passenger who made the request.
    """
...
def create_ride(request):
    ...
    Handles both publishing and saving as draft,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if publish_value == '1':
                ...
                messages.info(
                    request,
                    'Your ride has been saved as a draft.'
                )
            else:
            ...
            messages.error(
                request,
                'Please correct the errors below to create your ride.'
            )
    else:
        form = RideForm()
...
def edit_ride(request, ride_id):
    ...
    Only the driver can edit. Prevents saving as draft
    ...
    if ride.driver == request.user:
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                if request.POST.get('publish') == '1':
                    ...
                    if ride.ride_requests.exists():
                        messages.error(
                            request,
                            (
                                'You cannot save this ride as a draft because '
                                'there are ride requests for it.'
                            )
                        )
                        return render(
                            request,
                ...
                messages.error(
                    request,
                    'Please correct the errors below to update your ride.'
                )
        return render(
            request,
            'rides/edit_ride.html',
...
def delete_ride(request, ride_id):
    """
    Allow the ride creator to delete their ride listing,
    or cancel if accepted requests exist.
    If there are accepted requests, cancels the ride and
    notifies passengers instead of deleting.
    """
    ride = get_object_or_404(Rides, id=ride_id)
    if ride.driver == request.user:
        now = timezone.now()
        if ride.ride_requests.count() == 0 or ride.date < now:
            ...
            messages.add_message(
                ...
                messages.WARNING,
                (
                    'Ride cancelled. Passengers with ride requests '
                    'have been notified.'
                )
            )
        else:
            messages.add_message(
                ...
                messages.ERROR,
                (
                    'This cancelled ride cannot be deleted '
                    'because it has ride requests attached '
...
def my_ride_requests(request):
    ...
    grouped by status. Shows accepted, pending, and voided
    ...
    voided_requests = [
        r for r in all_requests if r.status in ['2', '3', '4', '5']
    ]
...
def edit_ride_request(request, request_id):
    ...
    Validates seat availability and updates both the ride
    and the request accordingly.
    """
    ride_request = get_object_or_404(
        RideRequest, id=request_id, passenger=request.user
    )
    ride = ride_request.ride
    old_seats = ride_request.seats_requested
    max_allowed = ride.seats_available + old_seats
    if request.method == 'POST':
        form = RideRequestEditForm(
            request.POST, instance=ride_request, max_seats=max_allowed
        )
        if form.is_valid():
            ...
            if new_seats > max_allowed:
                messages.error(
                    request,
                    f'Only {max_allowed} seats are available '
                    f'to request for this ride.'
                )
                return render(
                    request,
                    'rides/edit_ride_request.html',
                    {'form': form, 'ride_request': ride_request}
                )
            ...
            if seat_diff > 0:
                ...
                messages.success(
                    request,
                    f'Requested seat number updated to {new_seats}.'
                )
            else:
                # Only restore seats if the request was approved
        ...
        form = RideRequestEditForm(
            instance=ride_request, max_seats=max_allowed
        )
    return render(
        request,
        'rides/edit_ride_request.html',
...
def cancel_ride_request(request, request_id):
    ...
    Restores seats if the request was approved.
    Handles both pending and approved requests.
    ...
    if (
        request.user != ride_request.passenger and
    ...
    if request.method == 'POST':
        ...
        if ride_request.status == '1':
            ...
            messages.success(
                request,
                (
                    'The approved ride request was cancelled and the seat(s) '
                    'restored.'
                )
            )
            ...
            messages.success(
                request,
                'The pending ride request was cancelled.'
            )
        else:
            messages.error(
                request,
                'You cannot cancel a declined or already cancelled request.'
            )
...
def approve_ride_request(request, request_id):
    """
    Allow the driver to approve a pending ride request
    and deduct seats from the ride.
    ...
    if ride.seats_available < ride_request.seats_requested:
        messages.error(
            request,
            (
                f'Not enough seats available to approve this request. Only '
                f'{ride.seats_available} left.'
            )
        )
        return redirect('ride_detail', ride_id=ride.id)
    ...
    ride.seats_available -= ride_request.seats_requested
    ride.save()
    ride_request.status = '1'  # '1' means Accepted/Approved
    ride_request.save()
    messages.success(
        request,
        'Ride request approved and seats reserved.'
    )
    return redirect('ride_detail', ride_id=ride.id)
